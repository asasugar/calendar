Component({
  ...
  methods: {
    ...
    setMonth(setYear, setMonth, setDay) {
      if (this.data.selectDay.year !== setYear || this.data.selectDay.month !== setMonth) {
        ...
        this.setSpot();
        this.triggerEvent('onSelect', this.data.selectDay);
      }
    },
    // 展开收起
    openChange() {
      this.setData({
        ...
        this.triggerEvent('onToggleOpen', this.data.open);
      });
      this.dateInit();
      this.setSpot();
    },
  ...
  lifetimes: {
    attached() {
      const now = this.data.defaultTime ? new Date(this.data.defaultTime) : new Date();
      const selectDay = {
